...
st.markdown("""
    <style>
        ...
        /* Fundo Cinza para o App inteiro */
...
# ==========================================
# 2. LOCAIS E MATRIZ DE EMBARCAÇÕES
# ==========================================
locais_base = {
...
with col_exp1:
    with st.expander("⚠️ Reportar Perigo", expanded=False):
        with st.form("form_alerta"):
            tipo_alerta = st.selectbox("O que há na água?", ["🏊‍♂️ Banhistas no Canal", "🪵 Tronco Perigoso", "🏝️ Banco de Areia Oculto", "🎣 Rede de Pesca", "🚓 Fiscalização", "⚠️ Barco Enguiçado"])
            ...
            if st.form_submit_button("📢 Lançar no Mapa", type="primary"):
                ...
                conn = sqlite3.connect(banco_path); conn.execute("INSERT INTO alertas_waze (usuario, tipo_alerta, lat, lon, descricao) VALUES (?, ?, ?, ?, ?)", (st.session_state['usuario_email'], tipo_alerta, lat_a, lon_a, desc_alerta)); conn.execute("UPDATE perfis_usuarios SET pontos = pontos + 10 WHERE email = ?", (st.session_state['usuario_email'],)); conn.commit(); conn.close(); st.success("Alerta criado!")
    ...
    with st.expander("📍 Adicionar Novo Local", expanded=False):
        with st.form("form_novo_local"):
            ...
            if st.form_submit_button("💾 Catalogar", type="primary"):
                if novo_nome:
                    lat_n = locais_dinamicos[st.session_state['rota_destino']]["lat"] - 0.01; lon_n = locais_dinamicos[st.session_state['rota_destino']]["lon"] - 0.01
                    try:
                        conn = sqlite3.connect(banco_path); conn.execute("INSERT INTO locais_comunidade (nome, tipo, lat, lon, profundidade_est, criador) VALUES (?, ?, ?, ?, ?, ?)", (f"{novo_tipo.split(' ')[0]} {novo_nome}", novo_tipo, lat_n, lon_n, novo_prof, st.session_state['usuario_email'])); conn.execute("UPDATE perfis_usuarios SET pontos = pontos + 50 WHERE email = ?", (st.session_state['usuario_email'],)); conn.commit(); conn.close(); st.success("Local catalogado!"); st.rerun()
                    except sqlite3.IntegrityError: st.error("Nome já existe.")

# ==========================================
# 5. ABAS E INTELIGÊNCIA DE ROTA (CURVAS)
# ==========================================
...
with tab_mapa:
    ...
    if folga_agua < 0.5: st.error(f"🚨 **RISCO DE ENCALHE SEVERO:** O destino **{st.session_state['rota_destino']}** está muito raso ({profundidade_destino}m) para o seu barco ({calado_meu_barco}m de calado).")
    elif folga_agua < 1.5: st.warning(f"⚠️ **ATENÇÃO AO CASCO:** A área de destino ({profundidade_destino}m) está no limite para o seu barco ({calado_meu_barco}m de calado).")
    else: st.success(f"✅ **ROTA SEGURA:** Profundidade adequada no destino ({profundidade_destino}m). Boa navegação!")

    map_layer = st.radio("Visualização:", ["Satélite Real (Google)", "Ruas (Padrão Waze)", "Náutico Escuro"], horizontal=True, label_visibility="collapsed")
    ...
    # --- MOTOR DE WAYPOINTS (EVITANDO CRUZAR A CIDADE) ---
    zonas_oeste = ["Marina do David", "Praia da Lua", "Praia do Tupé", "Orla da Ponta Negra", "Flutuante Sedutor", "Meu Local Atual (GPS)"]
    zonas_leste = ["Porto de Manaus (Centro)", "Encontro das Águas"]

    lats_rota = [locais_dinamicos[origem]["lat"]]
    lons_rota = [locais_dinamicos[origem]["lon"]]

    # Se a rota for de Leste para Oeste (ou vice-versa), força a curva no rio
    if (origem in zonas_oeste and destino in zonas_leste) or (origem in zonas_leste and destino in zonas_oeste):
        if origem in zonas_oeste: # Descendo o rio
            lats_rota.extend([-3.090, -3.115, -3.130])
            lons_rota.extend([-60.080, -60.045, -60.035])
        else: # Subindo o rio
            lats_rota.extend([-3.130, -3.115, -3.090])
            lons_rota.extend([-60.035, -60.045, -60.080])
            ...
            with st.container():
                ...
                with c1:
                    if st.button(f"👍 Vi também ({row['upvotes']})", key=f"up_{row['id']}"):
                        conn = sqlite3.connect(banco_path); conn.execute(f"UPDATE alertas_waze SET upvotes = upvotes + 1 WHERE id = {row['id']}"); conn.commit(); conn.close(); st.rerun()
                with c2:
                    if st.button(f"👎 Já Sumiu ({row['downvotes']})", key=f"dw_{row['id']}"):
                        conn = sqlite3.connect(banco_path); conn.execute(f"UPDATE alertas_waze SET downvotes = downvotes + 1 WHERE id = {row['id']}"); conn.commit(); conn.close(); st.rerun()
                st.divider()
...
with tab_perfil:
    ...
    with st.form("form_perfil"):
        ...
        st.info(f"O seu barco tem um calado estimado em **{dicionario_embarcacoes.get(tipo_barco_edit, 1.0)} metros**. Usaremos isto para roteamento seguro.")
...
# ==========================================
# 6. RODAPÉ INSTITUCIONAL (EMERGÊNCIAS)
# ==========================================
st.markdown("""
    <div class="oficial-footer">
        <h4>📞 Contatos de Emergência (Órgãos Oficiais)</h4>
        <p>
            🚔 Polícia Militar: <strong>190</strong> &nbsp;&nbsp;|&nbsp;&nbsp; 
            🚑 SAMU (Urgência Médica): <strong>192</strong> &nbsp;&nbsp;|&nbsp;&nbsp; 
            🚒 Corpo de Bombeiros: <strong>193</strong>
        </p>
        <p>⚓ Capitania Fluvial da Amazônia Ocidental (Marinha): <strong>185</strong> ou (92) 2123-2222</p>
        <div class="creditos">
            <strong>Ygara Nav - O Waze dos Rios</strong> &copy; 2026<br>
            Desenvolvido com tecnologia de mapeamento colaborativo e IA Preditiva para segurança na navegação.<br>
            Apoio Institucional: ANA | CPRM | SEMA | Marinha do Brasil
        </div>
    </div>
""", unsafe_allow_html=True)
