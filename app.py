...
calha_rio_negro = [{"lat": -3.1430, "lon": -60.0150}, {"lat": -3.1300, "lon": -60.0400}, {"lat": -3.1150, "lon": -60.0550}, {"lat": -3.0950, "lon": -60.0800}, {"lat": -3.0850, "lon": -60.0950}, {"lat": -3.0500, "lon": -60.0700}, {"lat": -3.0305, "lon": -60.0570}, {"lat": -3.0400, "lon": -60.2500}]
calha_taruma = [{"lat": -3.0850, "lon": -60.0950}, {"lat": -3.0700, "lon": -60.1050}, {"lat": -3.0600, "lon": -60.0950}, {"lat": -3.0520, "lon": -60.1020}, {"lat": -3.0440, "lon": -60.1105}, {"lat": -3.0415, "lon": -60.1085}]
...
def gerar_rota_inteligente(origem_dict, destino_dict):
    ...
    for p in trilho_base:
        if min(o_lon, d_lon) - 0.01 <= p["lon"] <= max(o_lon, d_lon) + 0.01:
            if min(o_lat, d_lat) - 0.01 <= p["lat"] <= max(o_lat, d_lat) + 0.01: pontos_rota.append(p)
    pontos_rota.append({"lat": d_lat, "lon": d_lon})
    micropontos = []
    for i in range(len(pontos_rota)-1):
        p1, p2 = pontos_rota[i], pontos_rota[i+1]
        for step in range(10):
            f = step / 10.0; micropontos.append({"lat": p1["lat"] + (p2["lat"] - p1["lat"]) * f, "lon": p1["lon"] + (p2["lon"] - p1["lon"]) * f})
...
# ==========================================
# 3. ORGANIZAÇÃO EM GUIAS (TABS)
# ==========================================
tab_mapa, tab_alertas, tab_perfil = st.tabs(["🗺️ Mapa de Navegação", "⚠️ Reportar Perigo", "👤 Conta"])

# ---------------------------------------------------------
# GUIA 1: O MAPA E A NAVEGAÇÃO
# ---------------------------------------------------------
with tab_mapa:
    # CARD DE ROTEAMENTO
    ...
    if st.session_state['mostrar_restaurantes']:
        for nome, dados in restaurantes.items(): fig.add_trace(go.Scattermapbox(lat=[dados['lat']], lon=[dados['lon']], mode='markers+text', marker=go.scattermapbox.Marker(size=12, color="#FF9800"), text=["🍴 " + nome], textposition="bottom right", textfont=dict(color="#FF9800", size=13, weight="bold"), hoverinfo="text", hovertext=f"{nome}<br>{dados['desc']}"))
    if st.session_state['mostrar_postos']:
        for nome, dados in postos_combustivel.items(): fig.add_trace(go.Scattermapbox(lat=[dados['lat']], lon=[dados['lon']], mode='markers+text', marker=go.scattermapbox.Marker(size=12, color="#E53935"), text=["⛽ " + nome], textposition="bottom right", textfont=dict(color="#E53935", size=13, weight="bold"), hoverinfo="text", hovertext=f"{nome}<br>{dados['desc']}"))
    if st.session_state['mostrar_marinas']:
        for nome, dados in marinas.items(): fig.add_trace(go.Scattermapbox(lat=[dados['lat']], lon=[dados['lon']], mode='markers+text', marker=go.scattermapbox.Marker(size=12, color="#3949AB"), text=["⚓ " + nome], textposition="bottom right", textfont=dict(color="#3949AB", size=13, weight="bold"), hoverinfo="text", hovertext=f"{nome}<br>{dados['desc']}"))

    for nome, dados in locais_base.items():
        if nome != "Ponta Negra (Calha)": fig.add_trace(go.Scattermapbox(lat=[dados['lat']], lon=[dados['lon']], mode='markers+text', marker=go.scattermapbox.Marker(size=8, color="#757575"), text=[nome], textposition="top right", textfont=dict(color="#444", size=11), hoverinfo="text"))
...
# ---------------------------------------------------------
# GUIA 2: REPORTAR PERIGO
# ---------------------------------------------------------
with tab_alertas:
    st.markdown("### ⚠️ Avisar a Comunidade")
    st.write("Reporte perigos (Troncos, Bancos de areia, Fiscalização) para ajudar outros marinheiros.")
...
# ==========================================
# 4. RODAPÉ GLOBAL (FOOTER - PRESENTE EM TODAS AS TELAS)
# ==========================================
st.markdown("""
    <div class="footer-global">
        <div class="footer-emergencia">
            📞 <strong>EMERGÊNCIAS:</strong> 
            PM <strong>190</strong> &nbsp;|&nbsp; 
            SAMU <strong>192</strong> &nbsp;|&nbsp; 
            Bombeiros <strong>193</strong> &nbsp;|&nbsp; 
            Capitania Fluvial <strong>185</strong>
        </div>
        <div class="footer-creditos">
            <strong>Ygara Nav - O Waze dos Rios</strong> &copy; 2026<br>
            Mapeamento colaborativo e geolocalização fluvial da Bacia Amazônica.<br>
